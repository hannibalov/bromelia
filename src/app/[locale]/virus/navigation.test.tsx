...
vi.mock('@/../locales/client', () => ({
  ...
  useScopedI18n: (scope: string) => (key: string) => {
    const messages: Record<string, string> = {
      ...
      'games.virus.description': '¡Protege tus órganos!',
    };
...
describe('Virus Navigation', () => {
  it('shows the Virus game card on the platform home', () => {
    render(<HomePlatform />);
    expect(screen.getByRole('heading', { name: /Virus/i })).toBeInTheDocument();
  });

  it('has a link to /virus', () => {
    render(<HomePlatform />);
    const link = screen.getByRole('link', { name: /Virus/i });
    expect(link).toHaveAttribute('href', '/virus');
  });
});
